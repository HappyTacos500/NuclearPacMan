...
function updateBossAnimation() {
  ...
  var randomSquare = Math.floor(fixedRandom(level * currentUniverse) * (9)); // pick a square for the boss ghost to draw on
...
function buildWallsAndPellets() {
  ...
  for (var i = 0; i < NUM_ROWS * NUM_COLUMNS; i++) {
    var temp = fixedRandom(level * currentUniverse);   // random # between 0 and 1
...
function createItems() {
  ...
  for (i = 0; i < numPowerPellets; i++) {
    ...
    while (squareFound == false) {
      ...
      var ppSquare = Math.floor(fixedRandom(level * currentUniverse) * ((NUM_ROWS * NUM_COLUMNS) - 2)) + 1;  // return any square on board except 0.
...
function placePortal() {
  var portalPos = Math.floor(fixedRandom(level * currentUniverse) * (NUM_ROWS * NUM_COLUMNS)); // select a random square for the portal

  while (pellets[portalPos] == 0) { // Make sure the square to put the portal in is clear and keep moving it until it is
    portalPos = Math.floor(fixedRandom(level * currentUniverse) * (NUM_ROWS * NUM_COLUMNS));
  }
...
function respawnPacmanTimer() {
  ...
  for (i = 0; i < ghosts.length; i++) {
    ...
    while (checkZone == true) {
      ghostMoved = true;
      squareNum = Math.floor(fixedRandom(level * currentUniverse) * NUM_ROWS * NUM_COLUMNS);
...
function myGoodBombTimer() {
  ...
  while (foundSpot == false) {
    var targetSpot = Math.floor(fixedRandom(level * currentUniverse) * NUM_ROWS * NUM_COLUMNS);
...
function spawnAllGhosts() {
  ...
  for (var i = 0; i < NUM_GHOSTS; i++) {
    ...
    while (safe == false) {
      pos = Math.floor(fixedRandom(level * currentUniverse) * NUM_ROWS * NUM_COLUMNS);
...
function spawnGhost(squareNum) {
  ...
  var ghostRandomizer = ((Math.floor(fixedRandom(level * currentUniverse) * 8)) - 4) / 10;
  ...
  var tempGhostType = (Math.floor(fixedRandom(level * currentUniverse) * 3));  // number of ghost types = 3
  // var tempGhostType = 2;
...
function ghostRespawnTimer(type) {
  // spawn a new ghost
  reSpawnGhost(Math.floor(fixedRandom(level * currentUniverse) * (NUM_ROWS * NUM_COLUMNS)), type);
...
function calcGhostDirection(type, pos) {
  ...
  var random = Math.floor(fixedRandom(level * currentUniverse) * 100) + 1;
  ...
  if ((pacRow < ghostRow) && (pacCol == ghostCol)) {
    ...
    else // randomly go in 1 of the other 3 directions
    {
      ...
      var random2 = Math.floor(fixedRandom(level * currentUniverse) * 3);
  ...
  if ((pacRow < ghostRow) && (pacCol < ghostCol)) {
    ...
    if (random <= GHOST_SMARTS) {
      var random2 = Math.floor(fixedRandom(level * currentUniverse) * 2);
...
function checkIfOnGhostPacman(ghostId) {
  if ((ghosts[ghostId].squareNum == current) && (gameMode == GAME_MODE_POWER_ON)) {
    ...
    // spawn a new ghost
    reSpawnGhost(Math.floor(fixedRandom(level * currentUniverse) * (NUM_ROWS * NUM_COLUMNS)), ghosts[ghostId].ghost_type);  // return any square on board
...
function processPoison(ghostId, oldSquare) {
  ...
  if (ghosts[ghostId].ghost_type == GHOST_TYPE_POISON) {
    var check = Math.floor(fixedRandom(level * currentUniverse) * 100) + 1;
...
function updateTimer() {
  this.gameTimer -= 1;
  document.getElementById("timerVariable").innerHTML = gameTimer;
  if (gameTimer <= 0) {
    gameMode = GAME_MODE_OVER;
    // Game Over stop all game components
    for (var i = 0; i < ghosts.length; i++) {
